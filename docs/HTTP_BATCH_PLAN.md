...
截至当前实现，面板已提供 4 个原始需求的闭环入口：

1. 「预览：仅创建颜色分类」/「提交：仅创建颜色分类」
   - 对 Excel 中存在、页面中不存在的直播间编码，新增颜色分类规格值，并补对应 SKU 行。
   - 新 SKU 的 `code` 为空。
2. 「预览：仅填写商家编码」/「提交：仅填写商家编码」
   - 只处理页面已有颜色分类对应 SKU 的 `code`。
   - 页面没有该颜色分类时跳过，不新增规格。
3. 「预览：同时处理」/「提交：同时处理」
   - 页面缺颜色分类则新增颜色分类 + SKU，并写入商家编码。
   - 页面已有颜色分类则只更新对应 SKU 的 `code`。
4. 「检查编码映射」
   - 将当前页面解析出的颜色分类 / 商家编码与 popup 导入的 Excel 映射对比。
   - 输出匹配、缺少颜色分类、页面编码为空、编码不一致、页面多余颜色分类。
